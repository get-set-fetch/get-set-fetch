const Logger = require('../../logger/Logger').getLogger('BaseSite');
const UrlUtils = require('../..//utils/UrlUtils');
const PluginManager = require('../..//plugins/PluginManager');
const ExtractUrlPlugin = require('../..//plugins/process/ExtractUrlPlugin');
...
class BaseSite extends BaseEntity {
  ...
  constructor(name, url, opts, createDefaultPlugins = true) {
    ...
    if (!this.opts.resourceFilter) {
      this.opts.resourceFilter = BaseSite.defaultSiteOpts.resourceFilter;
    }
  ...
  /**
   * some plugins (like ChromeFetchPlugin) open external processes
   * each plugin is responsible for its own cleanup via cleanup
  */
  async cleanupPlugins() {
    for (let i = 0; i < this.plugins.length; i += 1) {
      await this.plugins[i].cleanup();
    }
  }

  get props() {
    return ['name', 'url', 'opts', 'robotsTxt', 'plugins'];
  }
  ...
  serialize() {
    ...
    serializedObj.opts = JSON.stringify(this.opts);
  ...
  fetchRobots(reqHeaders) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const robotsContentBuffer = (await fetchPlugin.fetch({ url: robotsUrl })).content;
        robotsContent = robotsContentBuffer ? robotsContentBuffer.toString() : null;
      }
      catch (err) {
        Logger.error(`err fetching robots.txt: ${robotsUrl}`);
        Logger.error(err);
      }
  ...
  crawlResource() {
    return new Promise(async (resolve, reject) => {
        ...
        if (this.plugins[i].test(resource)) {
          try {
            ...
            if (resource === null) {
              ...
              if (resource) {
                Logger.info(`${resource.url} selected for crawling`);
              }
            }
            else {
            ...
            Logger.error(`${this.plugins[i].constructor.name} ${resource ? resource.url : ''}`);
            Logger.error(err);
            reject(err);
            break;
          }
      ...
      if (resource) {
        Logger.info(`${resource.url} successfully crawled`);
      }
      resolve(resource);
    });
  }
  ...
  */
  ...
  initCrawl(opts) {
    ...
    if (opts.maxDepth) {
      this.addMaxDepthPlugin(opts.maxDepth);
    }
  ...
  addMaxDepthPlugin(maxDepth) {
    this.addPlugins([new ExtractUrlPlugin({ maxDepth })]);
  }
