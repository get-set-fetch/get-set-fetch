...
class KnexSite extends BaseSite {
  ...
  static createTable(table) {
    ...
    table.dateTime('robots_crawled_at');
  }
  ...
  static async get(nameOrId) {
    ...
    if (result) {
      // create JSON obj from requestHeaders text
      if (result.requestHeaders) {
        result.requestHeaders = JSON.parse(result.requestHeaders);
      }
      return Object.assign(new KnexSite(), result);
    }
    return null;
  }
    ...
    return KnexSite.knex.transaction(async (trx) => {
      ...
      urls.forEach((url) => {
        if (bloomFilter.test(url) === false) {
          resources.push(new KnexResource(this.id, url, depth));
          bloomFilter.add(url);
        }
      });
