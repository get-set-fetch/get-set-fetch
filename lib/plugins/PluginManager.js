...
// optional plugins
const PersistResourcePlugin = gsfRequire('lib/plugins/save/PersistResourcePlugin');
...
class PluginManager {
  ...
  static registerOptionals() {
    PluginManager.register(new PersistResourcePlugin());
    PluginManager.register(new ChromeFetchPlugin());
  }
  ...
  static add(newPlugins, currentPlugins) {
    ...
    newPlugins.forEach((newPlugin) => {
      PluginManager.validate(newPlugin);
    });
    ...
    return PluginManager.orderPlugins(newPlugins.concat(filteredCurrentPlugins));
  }
  ...
  static remove(toBeRemovedPluginNames, currentPlugins) {
    ...
    // return the final plugins in execution order
    return PluginManager.orderPlugins(filteredPlugins);
  }
