      ...
      lib
        .get(reqOpts, async (response) => {
          ...
          */
          ...
          /*
          convert stream data to 'utf8' if contentType is html or text
          for all other contentTypes work with the default Buffer object
          */
          let content = null;
          if (/(text|html)/.test(contentType)) {
            content = await NodeFetchPlugin.readUtf8Stream(response);
          }
          else {
            content = await NodeFetchPlugin.readBufferStream(response);
          }
